...
function fs_developer_perf_slow_queries_render_list(?array $slow_data): string
{
	...
	$out .= '<h4 class="title">' . esc_html__('Last recorded', 'fromscratch') . '</h4>';
	$out .= '<p class="description">';
	$out .= esc_html(sprintf(__('Page: %s', 'fromscratch'), $request_uri));
	if ($recorded_at !== '') {
		$out .= ' · ' . esc_html(sprintf(__('Recorded at %s', 'fromscratch'), $recorded_at));
	}
	$out .= '</p>';

	if ($message === 'no_slow_queries' && count($queries) === 0) {
		$threshold = fs_developer_perf_slow_queries_threshold();
		$out .= '<p>' . esc_html(sprintf(
			__('No queries over the threshold (%s s) were recorded for that page load.', 'fromscratch'),
			$threshold
		)) . '</p>';
		$out .= '<p class="description">' . esc_html__('Tip: Re-save the settings above to reinstall the recorder (db.php), then load any page while logged in as a developer.', 'fromscratch') . '</p>';
	} elseif (count($queries) > 0) {
		$groups = fs_developer_perf_slow_queries_group_by_type($queries);
		$group_labels = [
			'posts'    => __('posts', 'fromscratch'),
			'postmeta' => __('postmeta', 'fromscratch'),
			'terms'    => __('terms', 'fromscratch'),
			'options'  => __('options', 'fromscratch'),
			'other'    => __('other', 'fromscratch'),
		];
		$out .= '<p class="description" style="margin-top: 8px;"><strong>' . esc_html__('Query groups:', 'fromscratch') . '</strong> ';
		$parts = [];
		...
		if (count($top5) > 0) {
			$out .= '<p class="description" style="margin-top: 6px;"><strong>' . esc_html__('Top 5 slowest:', 'fromscratch') . '</strong></p>';
		...
		$out .= '<th style="width: 80px;">' . esc_html__('Time', 'fromscratch') . '</th>';
		$out .= '<th style="width: 140px;">' . esc_html__('Source', 'fromscratch') . '</th>';
		$out .= '<th>' . esc_html__('Query', 'fromscratch') . '</th></tr></thead><tbody>';
		foreach ($queries as $row) {
		...
		$out .= '<p>' . esc_html__('No recorded data.', 'fromscratch') . '</p>';
...
class FS_Wpdb_Query_Logger extends wpdb {
	...
	public static function save_and_clear(): void {
		$may_save = false;
		if (is_user_logged_in() && function_exists(\'fs_is_developer_user\') && fs_is_developer_user((int)get_current_user_id())) $may_save = true;
...
add_action('admin_bar_menu', function ($admin_bar): void {
	...
	if ($exp_queries_on && $exp_may_see) {
		...
		$title = $count > 0
			? sprintf(/* translators: %d = number of expensive queries */ __('Expensive queries (%d)', 'fromscratch'), $count)
			: __('Expensive queries', 'fromscratch');
		$admin_bar->add_node([
			'parent' => 'fs_wp_perf',
			'id'     => 'fs_wp_perf_slow_queries',
			'title'  => $title,
			'href'   => admin_url('options-general.php?page=fs-developer&fs_slow_queries=1'),
			'meta'   => ['title' => __('View last recorded slow queries', 'fromscratch')],
		]);
	}
	$admin_bar->add_node([
		'parent' => 'fs_wp_perf',
			...
			<div style="padding: 8px 10px;">
				...
				if ($is_developer && $exp_on && $exp_may_see) :
					...
					$exp_label = $exp_count > 0
						? sprintf(/* translators: %d = number of expensive queries */ __('Expensive queries (%d)', 'fromscratch'), $exp_count)
						: __('Expensive queries', 'fromscratch');
				?>
				<div style="margin-top: 8px; padding-top: 6px; border-top: 1px solid #c3c4c7;">
					<a href="<?= esc_url(admin_url('options-general.php?page=fs-developer&fs_slow_queries=1')) ?>" style="font-size: 11px;"><?= esc_html($exp_label) ?></a>
				</div>
				<?php endif; ?>
			</div>
