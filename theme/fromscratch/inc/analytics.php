...
{
    $week_start = $week_start->setTimezone(wp_timezone());
    $week_end = $week_start->modify('+6 days');
    $same_month = $week_start->format('Y-m') === $week_end->format('Y-m');
    ...
    return sprintf(
        /* translators: 1: start date, 2: end date */
        __('%1$s – %2$s', 'fromscratch'),
        wp_date((string) get_option('date_format'), $week_start->getTimestamp()),
        wp_date((string) get_option('date_format'), $week_end->getTimestamp())
    );
}
